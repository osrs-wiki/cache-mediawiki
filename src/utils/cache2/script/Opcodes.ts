/**
 * CS2 (ClientScript 2) Opcodes
 *
 * These are the instruction opcodes used in compiled OSRS client scripts.
 * Scripts are bytecode that runs on the client to handle UI, game logic, etc.
 */
...
/**
 * Check if an opcode uses a string operand
 */
export function isStringOpcode(opcode: number): boolean {
  return opcode === Opcodes.SCONST;
}

/**
 * Check if an opcode uses a byte operand instead of int
 */
export function isByteOperandOpcode(opcode: number): boolean {
  return (
    opcode === Opcodes.RETURN ||
    opcode === Opcodes.POP_INT ||
    opcode === Opcodes.POP_OBJECT ||
    opcode === Opcodes.PUSH_NULL ||
    opcode >= 100 // Most opcodes >= 100 use byte operands
  );
}

/**
 * Check if an opcode is a control flow jump
 */
export function isJumpOpcode(opcode: number): boolean {
  return (
    opcode === Opcodes.JUMP ||
